...
- Supporting full grammar of `fn`, `type`, or `struct/enum/union` or `impl` definitions (with
  generics and bounds), would be complicated. Invocations of such macros would be difficult to
  write. Difficult even to read, or to search in.
  
  And, migrating existing code would involve extra work.
- A macro can return a name/path of `fn`, `type` and `struct/enum/union` identifiers. Invocation of
  such a macro works in Rust in place of the identifier. (In most use cases it works out of the box,
  and in some it has easy workarounds, like putting the invocation between `<...>`). For example:
  ```rust
  ...
  ```
  However, there is no way to apply the same to `trait` items. For those we have a workaround:
  `use_with`.
...
[`Span::mixed_site()`] _are_ hygienic, so they can't be accessed from outside (of the macro that
created them).
...
You **can** include unicameral letters and use auto-detection if use also include some bicameral
letters and/or an underscore, and if those clearly imply the convention.
